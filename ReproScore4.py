...
# Gemini API key from environment (secret)
...
if filtered_citations:
    ...
    # Process with AI-powered text analysis (using Google Gemini API)
    ...
    try:
        ...
        test_response = requests.post(endpoint, params={"key": gemini_api_key}, json=test_payload)
        if test_response.status_code != 200:
            raise Exception(f"Gemini API test failed: {test_response.text}")
    ...
    for batch_start in range(0, len(abstracts_with_indices), batch_size):
        ...
        payload = {
            "system_instruction": {
                "parts": [{"text": "You are a scientific classifier. Respond with only: support (1), contradict (-1), or neutral (0). One per line for each provided abstract."}]
            },
            "contents": [
                {
                    "parts": [{"text": "\n\n".join(prompts)}]
